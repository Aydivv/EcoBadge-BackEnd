...
def get_businessProfile(
    ...
    response = schemas.BusinessProfile(id=b.id,name=b.name,address=b.address,postcode=b.postcode,description=b.description,cuisine=b.cuisine,number=b.pNumber,email=b.email,website=b.website,reviews=reviews,scores=scores,uber=b.uber,justeat=b.justeat)
    return response
...
def update_business(business: schemas.BusinessCreate, business_id: int, db: orm.Session=fastapi.Depends(services.get_db)):
    return services.update_business(db=db,id=business_id,business=business)
